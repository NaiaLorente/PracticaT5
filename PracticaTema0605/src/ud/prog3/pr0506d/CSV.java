...
public class CSV { 
	...
	private static void procesaLineaDatos( ArrayList<Object> datos ) {
		numLin++;
		UsuarioTwitter usu = new UsuarioTwitter(
				getString(datos, 0),
	            getString(datos, 1), 
	            getArrayListString(datos, 2),  
	            getString(datos, 3),
	            getLong(datos, 4), 
	            getLong(datos, 5), 
	            getString(datos, 6), 
	            getLong(datos, 7),  
	            getString(datos, 8),  
	            getArrayListString(datos, 9));
		GestionTwitter.addUsuarioID(usu.getId(), usu);
	//	System.out.println(GestionTwitter.getMapaPorID());
		
		GestionTwitter.addUsuarioNick(usu.getScreenName(), usu);
	//	System.out.println(GestionTwitter.getMapaPorNic());
	}
	
	private static String getString(ArrayList<Object> datos, int i) {
	    if (i >= 0 && i < datos.size()) {
	...
	private static ArrayList<String> getArrayListString(ArrayList<Object> datos, int i) {
	    if (i >= 0 && i < datos.size()) {
	        Object obj = datos.get(i);
	...
	private static Long getLong(ArrayList<Object> datos, int i) {
	    if (i >= 0 && i < datos.size()) {
	        Object obj = datos.get(i);
